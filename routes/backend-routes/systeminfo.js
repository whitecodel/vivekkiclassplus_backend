...
router.get('/systeminfo', NotLoggedIn, async (req, res) => {
    ...
    const publicIPv4tmp = await cmd('dig -4 TXT +short o-o.myaddr.l.google.com @ns1.google.com');
    const publicIPv4 = publicIPv4tmp.replace('\"', '').replace('\"', '').replace(' ', '');
    const publicIPv6tmp = await cmd('dig -6 TXT +short o-o.myaddr.l.google.com @ns1.google.com');
    const publicIPv6 = publicIPv6tmp.replace('\"', '').replace('\"', '').replace(' ', '');
