...
module.exports = {
  addnetjrfTest: async (req, res) => {
    ...
    try {
      const testRecords = await netjrfTest.find({
        name: req.body.name,
      });
      if (testRecords == "" || testRecords == null) {
        const postData = new netjrfTest({
          name: req.body.name,
        ...
        return res.send(
          "netjrf Test Name Already Exist. Please Add new Test Records!!"
        );
      }
    } catch (error) {
      console.log(error);
  ...
  questionView: async (req, res) => {
    const test = await netjrfTest
      .findOne({ _id: req.query.id }, { questions: 1 })
      .sort({
        _id: -1,
      });
    return res.render("admin/netjrftestquestion.ejs", {
    ...
    try {
      upload(req, res, async function (err) {
        ...
        for (let i = 0; i < sheets.length; i++) {
          ...
          try {
            const postResponse = await netjrfTest.findOneAndUpdate(
              { _id: req.body.id },
  ...
  addMoreQuestions: async (req, res) => {
    if (!req.body.id) return res.send("Please Pass netjrf Test id");

    // Checking test available in Document
    const testRecords = await netjrfTest
      .find({
        id: req.body.id,
      })
      .sort({
    ...
    if (testRecords == "" || testRecords == null)
      return res.send("No netjrf Test Records Found with that Test Id..");
    ...
    try {
      ...
      if (postResponse) {
        return res.send("netjrf Test Questions Added Successfully!!");
      }
    } catch (error) {
      return res.send("Something went Wrong!! Try Again..");
    }
  },
  updatenetjrfTest: async (req, res) => {
    ...
    try {
      const tests = await netjrfTest.findById(req.body.editid);
      if (tests) {
        const postResponse = await netjrfTest.findOneAndUpdate(
          filter,
          updateData
        ...
        return res.send("netjrf Test Records not Found.!!");
      }
    } catch (error) {
      return res.send("Something went Wrong!! Try Again..");
    }
  },
  updateQuestionnetjrfTest: async (req, res) => {
    if (!req.body.test_id)
      return res
        .status(404)
    ...
    try {
      ...
      if (postResponse) {
        return res.status(200).json({
          success: 1,
          message: "netjrf Test Questions Updated Successfully!!",
        });
      } else {
        return res.status(404).json({
          success: 0,
          message: "No netjrf Test Questions Record Found!!!",
        });
      }
    } catch (error) {
      return res
  ...
  getnetjrfTest: async (req, res) => {
    try {
      var category = req.query.id;

      if (category) {
        if (req.query.param == "questions") {
          const tests = await netjrfTest
          ...
          const tests = await netjrfTest
            ...
            .sort({ created_at: -1 });
          if (tests) {
            return res.render("admin/netjrftest", { tests });
          } else {
  ...
  deletenetjrfTest: async (req, res) => {
    try {
      var id = req.body.id;

      const test = await netjrfTest.findById(id);
      if (test) {
        const deleteResponse = await netjrfTest.deleteOne({ _id: id });
        if (deleteResponse) {
          return res.send("success");
        }
      } else {
      ...
      return res.send("Sorry!! netjrf Test deletion unsuccessful..");
    }
  },
  deleteQuestionnetjrfTest: async (req, res) => {
    try {
      var testId = req.body.testid;
      var quesId = req.body.quesid;

      const checkAvail = await netjrfTest.findOne(
        { _id: testId, "questions._id": quesId },
        { "questions.$": 1 }
      );

      if (checkAvail) {
        const deleteResponse = await netjrfTest.updateOne(
          { _id: testId },
        ...
        return res.send("netjrf Test Questions Records not Found.!!");
      }
    } catch (error) {
